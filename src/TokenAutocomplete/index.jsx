...
export default class TokenAutocomplete extends React.Component {
	...
	handleClick = e => {
		const clickedOutside = !ReactDOM.findDOMNode(this.wrapperEl).contains(e.target)
	...
	blur = () => {
		if (this.inputEl) {
			ReactDOM.findDOMNode(this.inputEl).blur()
		}
	...
	addSelectedValue = () => {
		...
		const newValue = areOptionsAvailable ? this.optionsEl.getSelected() : void 0
	...
	renderOptionsDropdown = () => {
		if (this.shouldShowOptions()) {
			...
			return <OptionList ref={node => this.optionsEl = node} {...passProps}/>
		} else {
			return null
		}
	...
	renderProcessing = () => {
		return this.props.processing ? <div ref={node => this.processingEl = node} style={this.props.style.processing} /> : null
	}

	renderFakePlaceholder = () => {
		return this.shouldShowFakePlaceholder()
			? (<div
				ref={node => this.fakePlaceholderEl = node}
	...
	renderDropdownIndicator = () => {
		return this.props.simulateSelect
			? <div ref={node => this.dropdownIndicatorEl = node} style={this.props.style.dropdownIndicator} />
			: null
	}

	render() {
		return (
			<div className={this.props.className} ref={node => this.wrapperEl = node} style={this.props.style.wrapper}>
				<div ref={node => this.inputWrapperEl = node} onClick={this.focus} style={this.props.style.inputWrapper}>
					{this.renderTokens()}
					{this.renderInput()}
